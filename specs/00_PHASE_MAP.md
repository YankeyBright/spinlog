...
| --- | --- | --- | --- |
| 0 | Product and Spec Lock | Freeze the pre-1.0 0.2 boundary, deferred features, package identity, target policy, and invariants. | `npm run check:phase0` |
...
- Phase 1 contains package-shell and release-preparation work only; it does not implement the pre-1.0 runtime.
- Phase 2 is the first runtime implementation phase, and tests ship with each runtime behavior.
- Prompts, structured logging, global write interception, stdin ownership, and advanced colors remain post-MVP. Explicit streams, target-local leases, custom frames, task groups, progress, intro, and outro are frozen 0.2 behavior.
