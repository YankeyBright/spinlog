...
**One-liner:** A zero-runtime-dependency, ESM-only terminal color and spinner primitive capped at 4,096 gzip bytes.
...
4. **Exact Size Budget:** `dist/index.js` is at most 4,096 bytes after gzip level 9.
...
7. **Stream Discipline:** Style helpers are side-effect-free and stream-free; spinner-owned output uses only stderr and the package never writes stdout.
...
The exact surface and behavior are frozen in `specs/v1-public-api.d.ts`, `specs/v1-styles-api.d.ts`, and `specs/v1-behavior.json`. They cover ANSI-16 styles, one interactive terminal lease, four mutable fields, static modes, terminal overrides, instance logging, explicit disposal, promise wrapping, and coordinated intro/outro flow messages. Phase 2 may implement no additional export.

Task groups, progress bars, prompts, structured logging, custom streams and animations, concurrent spinners, and advanced colors are deferred with rationale in `specs/16_POST_MVP_FEATURES.md`.
